...
class LoggingContext:
    """Context manager for temporarily modifying logger settings."""
...
def get_logger(name=__name__, level=logging.INFO) -> logging.Logger:
    """Create a multi-GPU friendly logger with rank_zero_only."""
    logger = logging.getLogger(name)
    logger.setLevel(level)

    for level in ('debug', 'info', 'warning', 'error', 'exception', 'fatal', 'critical'):
        setattr(logger, level, L.pytorch.utilities.rank_zero_only(getattr(logger, level)))

    return logger


class Sampler:
    """Base class for samplers with straight-through estimator."""
...
class GumbelSampler(Sampler):
    """Gumbel sampler using Gumbel-Max trick."""
...
class BinarySampler(GumbelSampler):
    """Binary sampler for binary choices."""
...
class GaussianSampler:
    """Gaussian sampler using reparameterization trick."""
